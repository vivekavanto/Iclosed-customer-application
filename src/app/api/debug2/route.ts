...
export async function GET() {
  ...
  const { data: tasks } = await supabaseAdmin.from("tasks_duplicate").select("*").eq("deal_id", deals?.id);
  const { data: milestones } = await supabaseAdmin.from("milestones_duplicate").select("*").eq("deal_id", deals?.id);
